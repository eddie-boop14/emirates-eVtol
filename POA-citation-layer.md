...
- **2026-08-23 · Distribution posture decided: organic only.** Owner's call — no LinkedIn, no newsletter, no contact collection of any kind. W3.3/W5.3 (briefing channel, LinkedIn page) are closed-won't-do; the Nov press pitch is optional and off by default. Growth rides the passive surfaces already shipped: launch pages at the demand spike, RSS/Atom (subscription without holding anyone's contacts), DOIs + Dataset Search + llms.txt + API/MCP for machine discovery. Same organic model that grew loisirs74.fr. Consequence accepted: slower, so the 1 Dec GSC checkpoint is the honest scoreboard.
